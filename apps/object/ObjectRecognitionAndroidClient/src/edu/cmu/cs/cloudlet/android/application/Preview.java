...
public class Preview extends SurfaceView implements SurfaceHolder.Callback {
	...
	public static final int FULLIMAGE_SIZE_WIDTH = 1024; // smallest size in
															// droid
	public static final int THUMBNAIL_SIZE_WIDTH = 320; // smallest size in
														// droid
	...
	public void surfaceCreated(SurfaceHolder holder) {
		...
		if (mCamera != null) {
			try {
				...
				parameters.setJpegThumbnailSize(THUMBNAIL_SIZE_WIDTH,
						thumbnailSize.height * THUMBNAIL_SIZE_WIDTH
								/ thumbnailSize.width);
				...
				Log.d("krha", "thumbnail size : (" + thumbnailSize.width + ", "
						+ thumbnailSize.height + ") --> " + +newSize.width
						+ ", " + newSize.height + ")");
				...
				for (Camera.Size size : list) {
					if (size.width < FULLIMAGE_SIZE_WIDTH)
						continue;
					if (difference > Math
							.abs((size.width - FULLIMAGE_SIZE_WIDTH))) {
						difference = Math
								.abs((size.width - FULLIMAGE_SIZE_WIDTH));
						closeSize = size;
					}
					Log.d("krha", "supporting fullimage size : " + size.width
							+ " " + size.height);
				}
				...
				Log.d("krha", "FullImage size : (" + newSize.width + ", "
						+ newSize.height + ")");
